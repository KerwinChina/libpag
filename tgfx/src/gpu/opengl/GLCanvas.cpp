...
void GLCanvas::drawAtlas(const Texture* atlas, const Matrix matrix[], const Rect tex[],
                         const Color colors[], const Opacity alpha[], size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    auto localMatrix = Matrix::I();
    auto scale = atlas->getTextureCoord(width, height);
    localMatrix.postScale(scale.x, scale.y);
    auto translate = atlas->getTextureCoord(tex[i].x(), tex[i].y());
    localMatrix.postTranslate(translate.x, translate.y);
    auto args = FPArgs(getContext(), Matrix::MakeScale(width, height));
    std::unique_ptr<FragmentProcessor> colorFP;
    std::unique_ptr<FragmentProcessor> maskFP;
    if (colors) {
      colorFP = Shader::MakeColorShader(colors[i], alpha[i])->asFragmentProcessor(args);
